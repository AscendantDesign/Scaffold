/*
	...
	*Added expand/collapse animation code
	...
	20200915.0907 - Updates by Daniel Patterson
	-	Set namespace to Scaffold.
	- Removed call to Graphics.Dispose() of the paint event argument during Paint event.
	- Commented all outlining regions to permantently disable code-hiding.
	-	Shrank the bar widths from 8 to 6.
*/
	...
	public class CollapsibleSplitter : System.Windows.Forms.Splitter
	{
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			if(this.Dock == DockStyle.Left || this.Dock == DockStyle.Right)
			{
				...
				if(hot)
				{
					g.FillRectangle(new SolidBrush(hotColor), new Rectangle(rr.X, rr.Y, 6, 115));
				}
				else
				{
					g.FillRectangle(new SolidBrush(this.BackColor), new Rectangle(rr.X, rr.Y, 6, 115));
				}

				// draw the top & bottom lines for our control image
				g.DrawLine(new Pen(SystemColors.ControlDark, 1), rr.X, rr.Y, rr.X + rr.Width, rr.Y);
				g.DrawLine(new Pen(SystemColors.ControlDark, 1), rr.X, rr.Y + rr.Height, rr.X + rr.Width, rr.Y + rr.Height);

				if(this.Enabled)
				{
					// draw the arrows for our control image
					// the ArrowPointArray is a point array that defines an arrow shaped polygon
					g.FillPolygon(new SolidBrush(SystemColors.ControlDarkDark), ArrowPointArray(rr.X, rr.Y + 3));
					g.FillPolygon(new SolidBrush(SystemColors.ControlDarkDark), ArrowPointArray(rr.X, rr.Y + rr.Height - 9));
				}
			...
			else if (this.Dock == DockStyle.Top || this.Dock == DockStyle.Bottom)
			{
				...
				if(hot)
				{
					g.FillRectangle(new SolidBrush(hotColor), new Rectangle(rr.X, rr.Y, 115, 6));
				}
				else
				{
					g.FillRectangle(new SolidBrush(this.BackColor), new Rectangle(rr.X, rr.Y, 115, 6));
				}

				// draw the left & right lines for our control image
				g.DrawLine(new Pen(SystemColors.ControlDark, 1), rr.X, rr.Y, rr.X, rr.Y + rr.Height);
				g.DrawLine(new Pen(SystemColors.ControlDark, 1), rr.X + rr.Width, rr.Y, rr.X + rr.Width, rr.Y + rr.Height);

				if(this.Enabled)
				{
					// draw the arrows for our control image
					// the ArrowPointArray is a point array that defines an arrow shaped polygon
					g.FillPolygon(new SolidBrush(SystemColors.ControlDarkDark), ArrowPointArray(rr.X + 3, rr.Y));
					g.FillPolygon(new SolidBrush(SystemColors.ControlDarkDark), ArrowPointArray(rr.X +  rr.Width - 9, rr.Y));
				}
			...
			//	DEP20200915.0905 - Don't dispose the graphics object used by the paint event
			//	arguments.
